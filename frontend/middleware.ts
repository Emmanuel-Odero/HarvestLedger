...
const authRoutes = ["/auth/signin", "/auth/signup", "/auth/trial", "/login"];
...
export function middleware(request: NextRequest) {
  ...
  const isInRegistrationFlow = registrationRoutes.some((route) =>
  ...
  if (token && isRegistrationComplete && tokenPayload?.role) {
    ...
    if (
      buyerRoutes.some((route) => pathname.startsWith(route)) &&
      userRole !== "buyer" &&
      userRole !== "admin"
    ) {
      return NextResponse.redirect(new URL("/dashboard", request.url));
